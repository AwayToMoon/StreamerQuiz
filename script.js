...
// AI Question Generation Function with multiple free API options
...
Wichtig: correct ist der Index (0-3) der richtigen Antwort. Antworte NUR mit dem JSON-Array, keine zusätzlichen Erklärungen.`;
    ...
    for (const api of apis) {
        try {
            ...
            const questions = await api.fn();
            if (questions && questions.length > 0) {
                console.log(`Successfully generated ${questions.length} questions using ${api.name}`);
                return questions;
            }
        } catch (error) {
...
async function tryOpenAIAPI(category, count, apiKey) {
    if (!apiKey) {
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
            ...
            messages: [
                ...
                {
                    role: 'user',
                    content: `Erstelle ${count} Quiz-Fragen zum Thema "${category}". Format: JSON-Array mit {question, answers: [4 Antworten], correct: 0-3}.`
                }
            ],
            temperature: 0.7,
            max_tokens: 2000
